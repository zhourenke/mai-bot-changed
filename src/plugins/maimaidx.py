...
async def _(event: Event, message: Message = EventMessage()):
    ...
    try:
        ...
        print(e)
        await spec_rand.finish("语法错误，请检查")
...
query_chart = on_regex(r"^## ([绿黄红紫白]?)([0-9]+)")
...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if len(argv) == 1 and argv[0] == '帮助':
        s = '''此功能为查找某首歌分数线设计。
命令格式：分数线 <难度><歌曲 ID> <分数线>
例如：分数线 紫799 100
命令将返回分数线允许的 TAP GREAT 容错以及 BREAK 50 落等价的 TAP GREAT 数。
以下为 TAP GREAT 的对应表：
Type GREAT/GOOD/MISS
TAP
        1/2.5/5
...
BREAK 50 落 (一共 {brk} 个) 等价于 {(break_50_reduce / 100):.3f} 个 TAP GREAT (-{break_50_reduce / total_score * 100:.4f}%)''')
        except Exception:
